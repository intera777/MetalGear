...
public class ItemView {
    ...
    private void loadItemSprites() {
        try {
            ...
            BufferedImage sheet =
                    ImageIO.read(getClass().getResource("/resources/weapon/ammo.png"));
            itemSprites = new BufferedImage[1]; // とりあえず要素数1
            itemSprites[0] = sheet;

            /*
             * 複数種類にする場合は以下のように分割 itemSprites[0] = sheet.getSubimage(0, 0, ITEM_SIZE, ITEM_SIZE);
    ...
    public void drawItems(Graphics g, int offsetX, int offsetY) {
        for (ItemModel item : itemsModel.getItems()) {
            ...
            if (type >= 0 && type < itemSprites.length && itemSprites[type] != null) {
                g.drawImage(itemSprites[type], drawX, drawY, ConstSet.TILE_SIZE, ConstSet.TILE_SIZE,
                        null);
            }
        }
    }
