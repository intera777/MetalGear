...
public class PlayerView {
    ...
    private void loadSprites() { // try...catchは例外処理. この存在により、強制終了を防ぐ.
        try {
            BufferedImage sheet = ImageIO.read(getClass().getResource(ConstSet.IMG_PATH_HEROIN));
            sprites = new BufferedImage[4][3]; // 4方向 × 3コマ

            for (int row = 0; row < 4; row++) {
                for (int col = 0; col < 3; col++) {
                    sprites[row][col] = sheet.getSubimage(col * ConstSet.PLAYER_SIZE, // キャラチップの列に関して32pxだけすらす
                            row * ConstSet.PLAYER_SIZE, // キャラチップの行に関して32pxだけずらす
                            ConstSet.PLAYER_SIZE, ConstSet.PLAYER_SIZE);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    protected void drawPlayer(Graphics g, int drawX, int drawY) {
        ...
        g.drawImage(sprites[row][col], drawX, drawY, ConstSet.PLAYER_SIZE, ConstSet.PLAYER_SIZE,
                null);
    }
}
